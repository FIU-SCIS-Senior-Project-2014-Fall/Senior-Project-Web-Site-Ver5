...
class UserController extends CI_Controller 
{
  ...
  public function index( )
	{
      ...
      if ($user_id)
			{
				if ($tempUser->isUserHeadProfessor($user_id))
				{
          $lRegularProjectsIds = $tempProject->getApprovedProjectsIds();
					$data[ 'title' ] = 'User Management';
					$this->load->view('admin_user_management', $data);
        }
      }
		}
	}
  ...
  public function edit_user( )
	{
       ...
       || !isset( $_POST['role' ] ) || !isset( $_POST[ 'email_address' ] ) )
		{
			$msg = 'Cannot edit user:
      ' . $_POST[ 'email_address' ] . '<br>There is missing information!';
      setErrorFlashMessage( $this, $msg );
		}
		else
		{
			if( isset( $_POST[ 'first_name' ] ) && isset( $_POST[ 'last_name' ] )
         && isset( $_POST[ 'email_address' ] ) && isset( $_POST[ 'role' ] ) && isset( $_POST[ 'userID' ] ) )
      {
  ...
  public function change_status( )
	{
		$data = array( );
		if( !isset( $_POST[ 'email_address' ] ) || !isset( $_POST[ 'userID' ] ) )
		{
			$msg = 'Cannot change user status: ' . $_POST[ 'email_address' ] . '<br>There is missing information!';
			setErrorFlashMessage( $this, $msg );
			redirect( 'user/filters' );
		}
		else
		{
			$data[ 'first_name' ] = $_POST[ 'first_name' ];
    ...
    //added function to handle the file uploads
    ...
    {     
        ...
        //the user is tying to upload a file of the wrong type
        if (($ext != "jpeg" && $ext != "jpg" && $ext != "gif" && $ext != "png")) 
        {   $msg = '';
            if ($ext != "")
                $msg = 'Cannot upload a .' . $ext. ' file. Please upload files of type: .png | .gif | .jpg';
            else 
        ...
        //user is trying to upload a supported file type
        else 
        {
            ...
            $src = checkUserUploadedPic($this, $spw_id);
            
            if($src != null)
            {
                //a file for this user has been stored, so delete the old copy...
                unlink('.' . $src);
            }
            ...
            {
                    //need to clear the picture field in the db, we will now use the uploaded picture
                    $this->spw_user_model->reset_picture($spw_id);
                    $data = array('upload_data' => $this->upload->data());
                    $msg = 'Your profile picture was successfully uploaded!';
                    setFlashMessage($this, $msg);
                    //need to refresh so that the new image has time to load...
                    redirect('/me', 'refresh');
            }
        }
    ...
    public function linkedIn_initiate()
    {
        // setup before redirecting to Linkedin for authentication.
         $linkedin_config = array(
             ...
             'callbackUrl'  => 'http://spws-dev.cis.fiu.edu/senior-project-website-v4/user/linkedIn_callback'
         );
...
//            die();
            ...
            $user_profile = (object)array(
                    ...
                    'positions_linkedIn'    => property_exists($user, 'positions')? ($user->positions->_total != 0 ? $this->parse_positions($user->positions):NULL ): null,
