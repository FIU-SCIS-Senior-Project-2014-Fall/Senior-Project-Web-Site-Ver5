...
//        $param[0] = $keyword;
//        
//        $sql = 'SELECT spw_user.id, spw_user.first_name, spw_user.last_name FROM spw_user WHERE (spw_user.first_name LIKE ?)';
//        //$this->db->select('spw_user.first_name');
//        //$this->db->where('spw_user.first_name'
//       // $this->db->like(('spw_user.first_name'), $options['keyword'], 'after');
//        $query = $this->db->query($sql, $param);//get('spw_user');       
//        
//        return $query->result();        
//   }
   ...
   public function isUserProfessor($spw_id)
   {
       $query = $this->db 
                       ->where ('id', $spw_id)
                       -> select('role')
       ...
       if($query -> num_rows() > 0)
       {
           if($query ->row() -> role == "HEAD" || $query ->row() -> role == "PROFESSOR")
               return true;
       }
       else
           return false;       
   ...
   public function reset_picture($spw_id)
   {
        $updateData = array ('picture' => '');
        $this->db->where('id', $spw_id);
        $this->db->update('spw_user', $updateData);
   }
	...
	/* Added to SPW v.3 for User Management */
	public function update_email_template( $id, $intro )
	{
    ...
    public function has_correct_credentials_and_is_inactive($email_address, $pwd)
    {
        $query = $this->db
                      ->where('email', $email_address)
                      ->where('hash_pwd', sha1( $pwd))
  ...
  public function create_new_user( $email_address, $role, $first_name, $last_name )
	{
		$data = array(
  ...
  /* Added to SPW v.3 for User Management System */
  public function edit_user( $email_address, $role, $firstname, $lastname, $id )
	{
		$data = array(
                  'email' => $email_address,
		...
		$this->db->update( 'spw_user', $data, "id = " . $id );
	}

  /* Added to SPW v.3 for User Management System */
	public function delete_user( $id )
	{
		return $this->db->delete( 'spw_user', "id = " . $id );
	}
	...
	/* Added to SPW v.3 for User Management System */
	public function bypass_activation( $id, $password )
	{
		$data = array( 'hash_pwd' => $password,
                  'status' => 'ACTIVE' );
		
		return $this->db->update( 'spw_user', $data, "id = " . $id );
	}
    ...
    public function create_new_professor_user($email_address, $password, $firstname, $lastname)
    {
        $data = array(
           'email' =>  $email_address ,
           'hash_pwd' =>  sha1($password),
    ...
    public function create_linkedin_profile($spw_id, $user_profile)
    {
        ...
        if(isset($user_profile->languages)){
            ...
            foreach ($user_profile->languages as $key => $value) {
                ...
                if($language_id == -1){
                    $this->spw_language_model->insert($value->name);
                    $language_id = $this->spw_language_model->get_language_by_name($value->name);
                }
                ...
                $sql = 'select spw_project.id, count(project_skills.skill) as nSkillMatch
                         from spw_project, (select skill
                                           ...
                                           where user = ?) as skills, (select spw_project.id, skill
                                                                       from spw_project, spw_skill_project
                                                                       where (spw_project.id = project) and (spw_project.status = "APPROVED"))
                                                                       as project_skills                                                                        
...
//        if (isset($query))
//        {
//            $lApprovedProjects = array();
//
//            foreach ($query->result() as $row)
//            {
//                $lApprovedProjects[] = $row->id;
//            }
//
//            return $lApprovedProjects;
//        }
//        }
...
//     public function isUserAMentor($user_id)
//    {
//        $param[0] = $user_id;
//        $sql = 'select role
//                from spw_user
...
//    }
    ...
    public function getMentorId($mentor)
    {
        $sql = 'select spw_user.id
                from spw_user
                where (role = "PROFESSOR") OR (role = "HEAD")';
        ...
        if ($users > 0)
        {
            for ($j = 0; $j < $users; $j++)
            {
                ...
                if($mentor == $this->get_fullname($row->id))
                {
                    return $row->id;
                }
            }
    ...
    public function getAllMentors($mentor)
    {
        $param[0] = $mentor;
        $sql = 'select id, first_name, last_name
                from spw_user
                where ((spw_user.role = "PROFESSOR") OR (spw_user.role = "HEAD")) AND (spw_user.id != ?) AND (spw_user.status = "ACTIVE")
...

    {
...

        {
            ...
            {
                if ($this->isUserProfessor($lUser[$i]))
                    $res[] = $lUser[$i];
...
//        die;
    ...
    public function allStudentsWithoutProj()
    {
        $sql = 'select id, first_name, last_name
                from spw_user
